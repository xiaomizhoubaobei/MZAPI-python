...
class Speed_128K:
    """
    EightK 类用于与百度ERNIE-Speed-128K模型进行交互。
    ...
    主要方法:
    - get_response: 发送请求到ERNIE-Speed-128K
    """
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            host_name="ERNIE-Speed-128K",
            token="kCrxvCIYEzhZfAHETXEB",
            peer_service="ERNIE-Speed-128K",
            peer_instance=" 180.97.107.95:443",
            peer_address=" 180.97.107.95",
            peer_ipv6="240e:ff:e020:934:0:ff:b0dc:3636",
            http_host="https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-speed-128k",
            server_name="MZAPI",
        )
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-Speed-128K") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-Speed-128K", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-Speed-128K")
            return X
