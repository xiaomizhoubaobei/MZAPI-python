...
class Lite_8K:
    """
    EightK 类用于与百度ERNIE-Lite-8K模型进行交互。
    ...
    主要方法:
    - get_response: 发送请求到ERNIE-Lite-8K
    """
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-Lite-8K",
            peer_instance=" 180.97.107.95:443",
            peer_address=" 180.97.107.95",
            peer_ipv6="240e:ff:e020:934:0:ff:b0dc:3636",
            http_host="https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-lite-8k",
            server_name="MZAPI",
        )
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-Lite-8K") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-Lite-8K", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-Lite-8K")
            return X
