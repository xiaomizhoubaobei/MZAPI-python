...
class Lite_8K:
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-Lite-Pro-128K",
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-Lite-Pro-128K") as span:
            url = (
                "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-lite-pro-128k?access_token="
                + self.access_token
            )
            payload = json.dumps(
                {
            ...
            self.log.start_process_log(response.json(), "ERNIE-Lite-Pro-128K", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-Lite-Pro-128K")
            return X
