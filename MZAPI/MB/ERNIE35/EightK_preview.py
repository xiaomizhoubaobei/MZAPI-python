...
class EightK_preview:
    """
    EightK_preview 类用于与百度ERNIE 3.5 8K-preview模型进行交互。
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-3，5-8K-preview",
    ...
    def get_response(
        ...
        with self.tracer.start_as_current_span("ERNIE-3.5-8K-preview") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-3.5-8K-preview", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-3.5-8K-preview")
            return X
