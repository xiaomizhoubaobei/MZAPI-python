...
class onetwentyeightk_preview:
    """
    onetwentyeightk_preview 类用于与百度ERNIE 3.5 128K-Preview模型进行交互。
    ...
    主要方法:
    - get_response: 发送请求到ERNIE 3.5 128K-Preview模型并获取响应
    """
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-3，5-128K-Preview",
    ...
    def get_response(
        ...
        """
        发送请求到ERNIE 3.5 128K-Preview模型并获取响应。
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-3.5-128K-Preview") as span:
            access_token = self.access_token.get_access_token()
            url = (
                "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-3.5-128K-Preview?access_token="
                + access_token
            )
            payload = json.dumps(
                {
            ...
            self.log.start_process_log(response.json(), "ERNIE-3.5-128K-Preview", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-3.5-128K-Preview")
            return X
